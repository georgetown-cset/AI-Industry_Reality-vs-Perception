...
```{r}
cbre_major <- read_csv("cbre_major.csv")

cbre_major <- arrange(cbre_major, desc(job_count))

# Remove null majors
cbre_major <- filter(cbre_major, !is.na(cbre_major$major))

# Horizontal bar graph, take the top 10 majors
ggplot(cbre_major[1:10,], aes(x = reorder(major, job_count), y = job_count)) + 
  geom_bar(stat = "identity") + 
  theme_gray(base_size = 15) +
  labs(title = "Top 10 Majors in Demand at cbre (2019)",
       x = "Major",
       y = "Job Count") +
  coord_flip()
```
...
```{r}
cbre_field <- read_csv("cbre_field.csv")

cbre_field <- arrange(cbre_field, DETAIL, desc(job_count))

# This helps set the order of the categories in the final graph
cbre_field$DETAIL <- factor(cbre_field$DETAIL,
                                levels = c("C1", "C2", "PT", "CT"))

# Bar graph of total job posticbres in each category
cbre_field %>%
  group_by(DETAIL) %>%
  summarise(total = sum(job_count)) %>%
  ggplot(aes(x = DETAIL, y = total)) + geom_bar(stat = "identity") +
  theme_gray(base_size = 15) +
  labs(title = "Total # of Job Postings by cbre in Each AI Workforce Category (2019)",
       x = "AI Workforce Category",
       y = "Job Count")

# Remove null majors
cbre_field <- filter(cbre_field, !is.na(major))
...
cbre_field %>%
  ...
  theme_gray(base_size = 15) +
  labs(title = "Top 5 Majors in Demand for C1 Jobs by cbre (2019)",
       x = "Major",
       y = "Job Count") +
  coord_flip()
...
cbre_field %>%
  ...
  ggplot(aes(x = reorder(major, job_count), y = job_count)) + geom_bar(stat = "identity") +
  theme_gray(base_size = 15) +
  labs(title = "Top 5 Majors in Demand for C2 Jobs by cbre (2019)",
       x = "Major",
       y = "Job Count") +
  coord_flip()
...
cbre_field %>%
  ...
  ggplot(aes(x = reorder(major, job_count), y = job_count)) + geom_bar(stat = "identity") +
  theme_gray(base_size = 15) +
  labs(title = "Top 5 Majors in Demand for PT Jobs by cbre (2019)",
       x = "Major",
       y = "Job Count") +
  coord_flip()
...
cbre_field %>%
  ...
  ggplot(aes(x = reorder(major, job_count), y = job_count)) + geom_bar(stat = "identity") +
  theme_gray(base_size = 15) +
  labs(title = "Top 5 Majors in Demand for CT Jobs by cbre (2019)",
       x = "Major",
       y = "Job Count") +
  coord_flip()
```
...
```{r}
cbre_trend <- read_csv("cbre_trend.csv")

ggplot(cbre_trend, aes(x = year, y = job_count, color = DETAIL)) +
  geom_line() + geom_point(size = 5) +
  scale_x_continuous(breaks=c(2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020)) +
  theme_gray(base_size = 15) +
  labs(title = "Trend in AI Labor Demand by cbre in Each AI Workforce Category",
       x = "Year",
       y = "Job Count") +
...
```{r}
cbre_trend_total <- read_csv("cbre_trend_total.csv")
...
cbre_trend_total %>%
  ...
  scale_x_continuous(breaks=c(2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020)) +
  theme_gray(base_size = 15) +
  labs(title = "Hiring Trend by cbre",
       x = "Year",
       y = "Job Count") +
...
```{r}
cbre_sector <- read_csv("cbre_sector.csv")
...
cbre_sector %>%
  ...
  ggplot(aes(x = canon_employer, y = percentage, fill = naics2_name)) +
    ...
    theme_gray(base_size = 15) +
    labs(title = "Percentage of Job Postings by cbre in Each Sector (2019)",
         y = "Percentage",
         fill = "Sector") +
