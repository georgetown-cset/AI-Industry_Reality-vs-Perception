...
```{r}
deloitte_major <- read_csv("deloitte_major.csv")

deloitte_major <- arrange(deloitte_major, desc(job_count))

# Remove null majors
deloitte_major <- filter(deloitte_major, !is.na(deloitte_major$major))

# Horizontal bar graph, take the top 10 majors
ggplot(deloitte_major[1:10,], aes(x = reorder(major, job_count), y = job_count)) + 
  geom_bar(stat = "identity") + 
  theme_gray(base_size = 15) +
  labs(title = "Top 10 Majors in Demand at Deloitte (2019)",
       x = "Major",
       y = "Job Count") +
  coord_flip()
```
...
```{r}
deloitte_field <- read_csv("deloitte_field.csv")

deloitte_field <- arrange(deloitte_field, DETAIL, desc(job_count))

# This helps set the order of the categories in the final graph
deloitte_field$DETAIL <- factor(deloitte_field$DETAIL,
                                levels = c("C1", "C2", "PT", "CT"))

# Bar graph of total job postings in each AI workforce category
deloitte_field %>%
  group_by(DETAIL) %>%
  summarise(total = sum(job_count)) %>%
  ggplot(aes(x = DETAIL, y = total)) + geom_bar(stat = "identity") +
  theme_gray(base_size = 15) +
  labs(title = "Total # of Job Postings by Deloitte in Each AI Workforce Category (2019)",
       x = "AI Workforce Category",
       y = "Job Count")

# Remove null majors
deloitte_field <- filter(deloitte_field, !is.na(major))
...
deloitte_field %>%
  ...
  theme_gray(base_size = 15) +
  labs(title = "Top 5 Majors in Demand for C1 Jobs by Deloitte (2019)",
       x = "Major",
       y = "Job Count") +
  coord_flip()
...
deloitte_field %>%
  ...
  ggplot(aes(x = reorder(major, job_count), y = job_count)) + geom_bar(stat = "identity") +
  theme_gray(base_size = 15) +
  labs(title = "Top 5 Majors in Demand for C2 Jobs by Deloitte (2019)",
       x = "Major",
       y = "Job Count") +
  coord_flip()
...
deloitte_field %>%
  ...
  ggplot(aes(x = reorder(major, job_count), y = job_count)) + geom_bar(stat = "identity") +
  theme_gray(base_size = 15) +
  labs(title = "Top 5 Majors in Demand for PT Jobs by Deloitte (2019)",
       x = "Major",
       y = "Job Count") +
  coord_flip()
...
deloitte_field %>%
  ...
  ggplot(aes(x = reorder(major, job_count), y = job_count)) + geom_bar(stat = "identity") +
  theme_gray(base_size = 15) +
  labs(title = "Top 5 Majors in Demand for CT Jobs by Deloitte (2019)",
       x = "Major",
       y = "Job Count") +
  coord_flip()
```
...
```{r}
deloitte_trend <- read_csv("deloitte_trend.csv")

ggplot(deloitte_trend, aes(x = year, y = job_count, color = DETAIL)) +
  geom_line() + geom_point(size = 5) +
  scale_x_continuous(breaks=c(2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020)) +
  theme_gray(base_size = 15) +
  labs(title = "Trend in AI Labor Demand by Deloitte in Each AI Workforce Category",
       x = "Year",
       y = "Job Count") +
...
```{r}
deloitte_trend_total <- read_csv("deloitte_trend_total.csv")

# NA = all unrelated AI jobs
# Replace NA with total number of jobs for each year
deloitte_trend_total %>%
  group_by(year) %>%
  ...
  scale_x_continuous(breaks=c(2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020)) +
  theme_gray(base_size = 15) +
  labs(title = "Hiring Trend by Deloitte",
       x = "Year",
       y = "Job Count") +
...
```{r}
deloitte_sector <- read_csv("deloitte_sector.csv")
...
deloitte_sector %>%
  ...
  ggplot(aes(x = canon_employer, y = percentage, fill = naics2_name)) +
    ...
    theme_gray(base_size = 15) +
    labs(title = "Percentage of Job Postings by Deloitte in Each Sector (2019)",
         y = "Percentage",
         fill = "Sector") +
