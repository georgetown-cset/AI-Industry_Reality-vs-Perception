...
ai_table AS (
  ...
  JOIN `gcp-cset-projects.ai_hype.services_sector` b ON entity.value = b.name
  WHERE language = "English"
  AND b.company_name = "Deloitte"
)
...
/* 
Looking at Deloitte AI articles with max/min entity scores overall
*/
WITH
deloitte_ai_table AS (
  SELECT id, duplicateGroupId, content, pubdate, entity, sentiment.score AS sentiment_score, b.company_name, b.naics2
  FROM `gcp-cset-projects.ai_hype.ai_articles` CROSS JOIN UNNEST(sentiment.entities) AS entity 
  JOIN `gcp-cset-projects.ai_hype.services_sector` b ON entity.value = b.name
  WHERE language = "English"
...
WHERE duplicateGroupID = 40155478153 ---max
OR duplicateGroupID = 40110575429 ---min