...
Case Study 5: Verizon Communications Incorporated
...
/* 
Looking at Verizon AI articles with max/min entity scores overall
*/
WITH
verizon_ai_table AS (
  SELECT id, duplicateGroupId, content, pubdate, entity, sentiment_score, b.company_name, b.naics2
...
SELECT duplicateGroupID, title, content, url FROM `gcp-cset-projects.gcp_cset_lexisnexis.raw_news` CROSS JOIN UNNEST(sentiment.entities) AS entity 
WHERE duplicateGroupID = 32902978771 ---max
OR (duplicateGroupID = 34469079382 AND entity.score = -3.25337) ---min