...
AND canon_employer = "Northrop Grumman"
...
ai_table AS (
  ...
  JOIN `gcp-cset-projects.ai_hype.manufacturing_sector` b ON entity.value = b.name
  WHERE language = "English"
  AND b.company_name = "Northrop Grumman"
)
...
/* 
Looking at Northrop Grumman AI articles with max/min entity scores overall
*/
WITH
ng_ai_table AS (
  SELECT id, duplicateGroupId, content, pubdate, entity, sentiment_score, b.company_name, b.naics2
...
WHERE duplicateGroupID = 32172254123 ---max
OR duplicateGroupID = 39495812235 ---min