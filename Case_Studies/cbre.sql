...
ai_table AS (
  ...
  JOIN `gcp-cset-projects.ai_hype.leasing_sector` b ON entity.value = b.name
  WHERE language = "English"
  AND b.company_name = "CBRE"
)
...
/* 
Looking at CBRE AI articles with max/min entity scores overall
*/
WITH
cbre_ai_table AS (
  SELECT id, duplicateGroupId, content, pubdate, entity, sentiment_score, b.company_name, b.naics2
...
WHERE duplicateGroupID = 38836010120 ---max
OR (duplicateGroupID = 37792835136 AND entity.score = -2.07545) ---min